...
export async function createSourceFromUrl(input: {
  ...
  try {
    return await createSourceFromImportedJsonInternal(
      sourceUrl,
      input.playlistId,
...
export async function createSourceFromImportedJson(input: {
  adminSecret: string;
  playlistId: string;
  playlistVersion: number;
  sourceUrl: string;
  ...
  try {
    ...
    console.error(
      "[createSourceFromImportedJson] failed:",
      input.sourceUrl,
      error,
    );
    return { ok: false, error: asErrorMessage(error) };
...
async function createSourceFromImportedJsonInternal(
  sourceUrl: string,
  playlistId: string,
  playlistVersion: number,
  importedJson: unknown,
  ...
  const created = await db.transaction(async (tx) => {
    const playlistResult = await tx
      .update(playlists)
      .set({ version: playlistVersion + 1, updatedAt: now })
      .where(
        and(
          eq(playlists.id, playlistId),
          eq(playlists.version, playlistVersion),
    ...
    const maxSortResult = await tx
      ...
      .where(
        and(
          eq(sources.playlistId, playlistId),
  ...
  await logMutation("source.create", summary, playlistId);
  revalidatePath("/");
  revalidatePath(`/playlist/${playlistId}`);
  revalidateTag("playlists");
